// $Id: setup.cpp 5 2011-08-09 02:08:58Z kawano $
/******************************************************************************/
/*  setup.cpp                                                                 */
/*        initialize all quantities in nucleus and GDR parameters             */
/******************************************************************************/
...
void statGenerateLevels(int nlev, Nucleus *n, LevelDensity *ldp)
{
  ...
  for(int i=0 ; i<nlev ; i++)
    elev[i] = (i==nlev) ? elmax : genrand_real3() * (elmax-elmin) + elmin;
  sort(elev,elev+nlev);
...
double normaldist(double m, double s)
{
  double a,b,r;
  do{
    a = 2.0*genrand_real1()-1.0;
    b = 2.0*genrand_real1()-1.0;
    r  = a*a+b*b;
  }while (r>=1.0 || r==0.0);
