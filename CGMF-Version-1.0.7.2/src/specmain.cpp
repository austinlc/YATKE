// $Id: specmain.cpp 5 2011-08-09 02:08:58Z kawano $
/******************************************************************************/
/*  specmain.cpp                                                              */
...
//static void    specPopulationBypass(int, double, Nucleus *, double *);
static void    specPopulationBypass(int, double, Nucleus *, double *, int); // Ionel's version 6/25/13
static void    specPopulationBypass(int k0, double dp, Nucleus *n, double *spc, int jspin , int parity ); // Ionel, 8/29/2013
...
/**********************************************************/
void specMain(double initJ, int initP, double spinf, double **spc)
{
  Transmission  tin;
                          ...
                          double **tg, double **spc)
{
  ...
  for(int j0=i0 ; j0<MAX_J*2+i0 ; j0+=2){
    ...
    for(int p0=-1 ; p0<=1 ; p0+=2){
      ...
      if(tsum==0.0 && pop>0.0){
        //        specPopulationBypass(k0,pop,&ncl[c0],spc[0]);
        //        specPopulationBypass(k0,pop,&ncl[c0],spc[0], jdx); // Ionel
        specPopulationBypass(k0,pop,&ncl[c0],spc[0], jdx, p0); // Ionel
        continue;
      }
                             ...
                             int jspin , int parity )
{
  ...
  for( int kk2 = 0 ; kk2 <  n->ndisc ; kk2++ ){
    ...
    if( fabs( n->lev[kk2].spin - jspin ) < 2. && 1. <= n->lev[kk2].spin +
       jspin ){
  ...
  if(kk0 == -1 ){
    for( int kk2 = 0 ; kk2 <  n->ndisc ; kk2++ ){
      ...
      //    cout << fabs( n->lev[kk2].spin - jspin ) << " " << delJ << endl ;
    }
  }
  eg -= n->lev[kk0].energy ;
...
/**********************************************************/
//void    specPopulationBypass(int k0, double dp, Nucleus *n, double *spc)
void    specPopulationBypass(int k0, double dp, Nucleus *n, double *spc, int jspin) // Ionel, 6/25/2013
{
  double eg = n->excitation[k0];
  
  // Ionel's version, 6/25/2013
  int kk0 = -1 ; double delJ = 1000. ;
